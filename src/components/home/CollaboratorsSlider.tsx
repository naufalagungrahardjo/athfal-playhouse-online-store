  ...
  return <section className="py-6 bg-white">
      <div className="athfal-container">
        ...
        // speed: 10, // <-- Removed invalid property
          ...
          <CarouselContent>
            {collaborators.map((c, idx) => <CarouselItem key={c.name + idx} className="basis-auto px-4 flex items-center justify-center" style={{
            width: 220,
            maxWidth: 240
          }} // around 210 + padding
          >
                <img src={c.logo} alt={c.name} title={c.name} className="object-contain transition-all h-[35px] w-[210px] sm:h-[38px] sm:w-[220px] bg-white rounded-lg shadow border border-gray-100 hover:scale-105" style={{
              minWidth: 120,
              minHeight: 25
            }} />
              </CarouselItem>)}
