...
const DEFAULT_COPY: WebsiteCopy = {
  homePage: {
    ...
    aboutDescription: {
      id: "Athfal Playhouse adalah kelas bermain anak yang menggabungkan metode bermain sambil belajar dengan nilai-nilai Islam.",
  ...
  paymentConfirmation: {
    ...
    description: {
      id: "Setelah melakukan pembayaran, mohon kirimkan bukti pembayaran Anda ke WhatsApp admin kami dengan menekan tombol di bawah ini.",
      en: "After making the payment, please send your payment proof to our admin via WhatsApp by clicking the button below.",
    },
  },
  checkoutTerms: {
    title: { id: "Syarat & Ketentuan", en: "Terms & Conditions" },
    content: {
      id: "Dengan melanjutkan pemesanan ini, saya menyetujui syarat dan ketentuan yang berlaku.",
      en: "By proceeding with this order, I agree to the applicable terms and conditions.",
    },
  },
};
...
async function fetchWebsiteCopy(): Promise<WebsiteCopy> {
  const { data, error } = await supabase.from("website_copy").select("content").eq("id", "main").maybeSingle();
